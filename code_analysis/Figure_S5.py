...
var = 'disequilibrium_area_weighted_mean'
...
# Disequilibrium
# global
df = pd.read_csv(path + 'PyGEM_global_stats_median_a.csv')

temp_ch_ipcc = df['temp_ch_ipcc'].values
disequilibrium = np.full((len(temp_ch_ipcc), len(regions) + 1), np.nan)

disequilibrium[:, 0] = df[var].values

# regional
for i, region in enumerate(regions):

    if region in {1, 3, 4, 5, 7, 9, 17, 19}:
        df = pd.read_csv(path + f'PyGEM_regional_stats_{region:02d}_median_a.csv')
    else:
        df = pd.read_csv(path + f'PyGEM_regional_stats_{region:02d}_median_k.csv')

    disequilibrium[:, i + 1] = df[var].values

#%% Lowess fit
# global
df = pd.read_csv(path + f'PyGEM_global_stats_{var}_median_a_lowess_fit.csv')
...
for i, region in enumerate(regions):
    ...
    if region in {1, 3, 4, 5, 7, 9, 17, 19}:
        df = pd.read_csv(path + f'PyGEM_regional_stats_{region_2d}_{var}_median_a_lowess_fit.csv')
    else:
        df = pd.read_csv(path + f'PyGEM_regional_stats_{region_2d}_{var}_median_k_lowess_fit.csv')
...
for region in range(20):
    ...
    # Only left column has y-axis labels
    if c == 0:
...
fig.text(0.015, 0.50, r'Area-weighted mean $\alpha$', ha='center', va='center',
...
out_pdf = '/Users/wyan0065/Desktop/OGGM/disequilibrium/figures/figure_S5.png'
