...
wgms_data = wgms_data.dropna(subset=['RGIId', 'AAR_steady']).copy()
loibl_AAR_mean_data = loibl_data.dropna(subset=['RGIId', 'AAR_mean']).copy()
loibl_AAR_steady_data = loibl_data.dropna(subset=['RGIId', 'AAR_steady']).copy()


# %% add PyGEM AAR_steady data
nc_file = pygem_path + 'PyGEM_global_glacier_stats_median_a.nc'

data = xr.open_dataset(nc_file)
rgi_id = data['rgi_id'].values.astype(str)
pygem_AAR_mean = data['AAR_mean'].isel(experiment=0).values
pygem_AAR_steady = data['AAR_steady'].isel(experiment=0).values

rgi_area_km2 = data['rgi_area_km2'].values

pygem_data = pd.DataFrame({
    'RGIId': rgi_id,
    'pygem_AAR_mean': pygem_AAR_mean,
    'pygem_AAR_steady': pygem_AAR_steady,
...
loibl_AAR_mean_compare = loibl_AAR_mean_data.merge(
    pygem_data,
    on='RGIId',
    how='inner'
)

loibl_AAR_steady_compare = loibl_AAR_steady_data.merge(
    pygem_data,
    on='RGIId',
    how='inner'
)

wgms_compare = wgms_compare.dropna(subset=['AAR_steady', 'pygem_AAR_steady']).copy()
loibl_AAR_mean_compare = loibl_AAR_mean_compare.dropna(subset=['AAR_mean', 'pygem_AAR_mean']).copy()
loibl_AAR_steady_compare = loibl_AAR_steady_compare.dropna(subset=['AAR_steady', 'pygem_AAR_steady']).copy()
...
for i, ax in enumerate([ax_c, ax_d]):
    ...
    if i == 0:
        compare_data = loibl_AAR_mean_compare
        title = 'Loibl-nowline observations$^{17}$'
        x_col = 'pygem_AAR_mean'
        y_col = 'AAR_mean'
    else:
        compare_data = loibl_AAR_steady_compare
        title = 'Loibl-nowline observations$^{17}$'
        x_col = 'pygem_AAR_steady'
        y_col = 'AAR_steady'

    x = compare_data[x_col].values
    y = compare_data[y_col].values
    ...
    if p_value < alpha:
        p_txt = f'paired t-test: {p_value:.1g}$^*$'
    else:
...
out_pdf = '/Users/wyan0065/Desktop/OGGM/disequilibrium/figures/figure_S10.png'
