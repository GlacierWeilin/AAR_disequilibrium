...
# ====== Only marine- and land-terminating ======
find_id = np.where(data['is_tidewater'] == 1)[0]
marine = xr.DataArray(
    np.column_stack((a[find_id], AAR_mean[find_id], AAR[find_id])),
    coords=[np.arange(0, len(find_id)), ['a', 'AAR_mean', 'AAR']],
    dims=['samples', 'variables']
)
...
categories = ['marine','land']
frames = []
for cat, data in zip(categories, [marine,land]):
...
ax1.set_position([pos1.x0-0.03, pos1.y0 -0.04, pos1.width + 0.04, pos1.height + 0.15])
...
for i, category in enumerate(categories):
    ...
    ax1.errorbar(
        ...
        marker='o', markersize=3, capsize=2, color=colors[i], linestyle='none'
    )
    ax1.plot(
        [x_positions[0] + i * category_offset], [a_data.median()],
        marker='_', markersize=8, color=colors[i], linestyle='none'
    )
...
ax2.errorbar(
    ...
    marker='o', markersize=3, capsize=2, color='dimgrey', linestyle='none'
)
ax2.plot(
    x_positions[0] -loc, 0.28,
    marker='_', markersize=6, color='dimgrey', linestyle='none'
)

ax2.text(x_positions[2]-2.2, 0.90, 'B', color='k', fontweight='bold', ha='center', va='center', size=12)
...
ax2.text(x_positions[0]+1.6, 0.28, 'median', color='dimgrey', ha='left', va='center', size=6)
ax2.text(x_positions[0]+1.6, 0.31, 'mean ± 1σ', color='dimgrey', ha='left', va='center', size=6)

ax2.text(x_positions[0]+1.45, 0.172, 'Glacier type:', color='k', ha='left', va='center', size=6)

handles = [plt.Line2D([0], [0], color=colors[i], lw=4) for i in range(len(categories))]
ax2.legend([handles[0],handles[1]],
                  ['Marine-terminating','Land-terminating'],
                  loc='lower left', ncol=1, columnspacing=1,frameon=False, handlelength=1, bbox_to_anchor=(0.63, -0.025), labelspacing=0.05)
